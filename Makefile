...
format:
	echo "starting $@ process using $(SHELL) as user $$(whoami)"
	source ./ci/format.sh && format_all

.ONESHELL:
lint:
	echo "starting $@ process using $(SHELL) as user $$(whoami)"
	source ./ci/lint.sh && lint_all

.ONESHELL:
test_all_docker:
	sudo echo "starting $@ process using $(SHELL) as user $$(whoami)"
	source ./ci/test.sh "serial" && execute_tests

.ONESHELL:
test_spec:
	sudo echo "starting $@ process using $(SHELL) as user $$(whoami)"
	source ./ci/test.sh "serial" && execute_test $(spec_file) $(use_docker)
