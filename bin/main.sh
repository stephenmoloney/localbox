...
function install() {
    ...
    # Run first
    bash "${PROJECT_ROOT}/bin/install/asdf.sh" "${ASDF_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/asdf_plugins.sh"
    bash "${PROJECT_ROOT}/bin/install/azure_cli.sh" "${AZURE_CLI_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/debian_pkgs.sh"
    bash "${PROJECT_ROOT}/bin/install/docker.sh" "${DOCKER_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/docker_compose.sh" "${DOCKER_COMPOSE_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/dotnet_core.sh" "${DOTNET_CORE_SDK_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/go.sh" "${GO_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/krew.sh" "${KREW_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/rust.sh" "${RUST_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/rust_pkgs.sh"
    bash "${PROJECT_ROOT}/bin/install/shellcheck.sh" "${SHELLCHECK_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/shfmt.sh" "${SHFMT_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/tmuxinator.sh" "${TMUXINATOR_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/yamllint.sh" "${YAMLLINT_VERSION}"

    # Run second (may have dependencies on first run)
    bash "${PROJECT_ROOT}/bin/install/tmux_plugin_manager.sh" "${TMUX_PLUGIN_MANAGER_VERSION}"
    bash "${PROJECT_ROOT}/bin/install/vim.sh" \
