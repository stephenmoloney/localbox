...
PDF2ODT_VERSION_FALLBACK=1.0.0
...
function get_current_version() {
    pip show pdf2odt | awk NR==2 | cut -d' ' -f2 2>/dev/null || true
}

function install_pdf2odt() {
    local version="${1}"
...
function main() {
    local version="${1:-$PDF2ODT_VERSION_FALLBACK}"

    install_pdf2odt "${version}"
}
